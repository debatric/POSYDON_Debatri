...
class Pulsar:
    ...
    def __init__(self, initial_mass, B_field=float("NaN"), spin=float("NaN"), P_dot=float("NaN"), alpha = 30.0):
        ...
        NS_RADIUS = CO_radius(initial_mass, "NS")*astro_const.R_sun  # POSYDON constant for NS radius [m] 
        
        self.mass = initial_mass*astro_const.M_sun            # mass of the NS [kg]
    ...
    def calc_moment_of_inertia(self):
        ...
        '''
        M = self.mass/astro_const.M_sun       # mass of the NS [Msun] 
        R = self.radius/astro_const.R_sun     # radius of the NS [Rsun]

        return 2/7 * (1 - 2.42e-6*M/R - 2.9e-12*M**2/R**2)**-1 * M*R**2 * (astro_const.M_sun*astro_const.R_sun**2)
    ...
    def calc_NS_edd_lim(self, surface_h1):
        ...
        Mdot_edd = (4 * np.pi * astro_const.m_p * self.radius * astro_const.c) / astro_const.sigma_T 
        return Mdot_edd / (astro_const.M_sun/const.secyer)      # convert [kg/s] to [Msun/yr]
    ...
    def calc_NS_luminosity(self):
        ...
        """
        kpc2cm = astro_const.kpc    # kpc to m
    ...
    def calc_NS_spindown_rate(self):
        ...
        omega_dot = - (8 * np.pi * (self.Bfield**2) * (self.radius**6) * (self.spin**3) * (np.sin(alpha))**2) / (3 * astro_const.mu0 * (astro_const.c**3) * I)
    ...
    def calc_magnetosphere_radius(self, Mdot_acc):
        ...
        '''
        Mdot_acc *= (astro_const.M_sun/const.secyer)  # convert [Msun/yr] to [kg/s]
        ...
        r_m = ((2 * np.pi**2 * self.radius**12 * self.Bfield**4)/(astro_const.G * astro_const.mu0**2 * Mdot_acc**2 * self.mass))**(1/7)
    ...
    def calc_NS_spin_equilibrium(self, Mdot_acc):
        ...
        omega_eq = 1/(2*np.pi) * (astro_const.G*self.mass/r_m**3)**(1/2) # equillibrium spin [1/s]
    ...
    def calc_NS_spin_limit(self, C = 1.15):
        ...
        f_k = C * np.sqrt(self.mass/astro_const.M_sun) * (R_ns**(-3/2)) # unints of kHz
    ...
    def Vdiff_fnct(self, R_mag):
        ...
        Omega_k = np.sqrt(astro_const.G*self.mass/R_mag**3) # from COMPAS function [Hz]
    ...
    def detached_evolve(self, delta_t, tau_d):
        ...
        mu_0 = astro_const.mu0          # permeability of free space [N/A^2]
        ...
        # evolve the NS spin
        A = 8 * np.pi * R**6 * np.sin(alpha)**2 / (3 * mu_0 * astro_const.c**3 * I)    # [s^5 A^2/kg^2]
    ...
    def RLO_evolve_CMC(self, delta_t, tau_d, delta_M, delta_Md):
        ...
        G = astro_const.G             # gravitational constant [c^3 kg^-1 s^-2]
        mu_0 = astro_const.mu0        # permeability of free space [N/A^2]
        tau_d *= (1e9*const.secyer)   # B-field decay timescale [s]
        delta_Md *= astro_const.M_sun # magnetic field mass decay scale [kg]

        delta_M *= astro_const.M_sun  # mass accreted by pulsar during RLO [kg]
    ...
    def RLO_evolve_COMPAS(self, delta_M, delta_Md, Mdot_acc, CE):
        ...
        '''
        G = astro_const.G             # gravitational constant [m^3 kg^-1 s^-2]
        delta_Md *= astro_const.M_sun # magnetic field mass decay scale [kg]
        B_min = 1e8*1e-4              # minimum Bfield strength at which Bfield decay ceases [T]
        mu_0 = astro_const.mu0        # permeability of free space [N/A^2]
        efficiency = 1                # set the pulsar to the maximum accretion efficiency to start

        delta_M *= astro_const.M_sun  # convert Msun to kg
